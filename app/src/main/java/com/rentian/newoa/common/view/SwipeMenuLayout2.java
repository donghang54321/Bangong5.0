...
/**
 * Created by rentianjituan on 2018/3/22.
 */
...
public class SwipeMenuLayout2 extends ViewGroup {
    private static final String TAG = "zxt/SwipeMenuLayout2";
    ...
    //存储的是当前正在展开的View
    private static SwipeMenuLayout2 mViewCache;
    ...
    public SwipeMenuLayout2 setHuadong(boolean huadong) {
     ...
     * @param ios
     */
    public SwipeMenuLayout2 setIos(boolean ios) {
        isIos = ios;
        return this;
    }
     ...
     * @return
     */
    public SwipeMenuLayout2 setLeftSwipe(boolean leftSwipe) {
     ...
     * @return
     */
    public static SwipeMenuLayout2 getViewCache() {
        return mViewCache;
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SwipeMenuLayout2, defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            //如果引用成AndroidLib 资源都不是常量，无法使用switch case
            if (attr == R.styleable.SwipeMenuLayout2_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.SwipeMenuLayout2_ios) {
                isIos = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.SwipeMenuLayout2_leftSwipe) {
...
/*                    if (!mScroller.isFinished()) {
    ...
    public void smoothExpand() {
        ...
        invalidate();*/
        //展开就加入ViewCache：
        mViewCache = SwipeMenuLayout2.this;

        //2016 11 13 add 侧滑菜单展开，屏蔽content长按
        if (null != mContentView) {
            mContentView.setLongClickable(false);
        }
        MyApp.getInstance().setZhankai(true);
        cancelAnim();
